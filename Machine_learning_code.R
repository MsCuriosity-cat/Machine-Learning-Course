...
# generate trainging and test sets
...
# download the mnist digits dataset from the dslabs package
mnist <- read_mnist()
view(mnist)
ncol(mnist$test$images)
...
bind_rows(guessing, height_cutoff) %>%
  ggplot(aes(recall, precision, colour = method)) +
  geom_line() +
  geom_point()
